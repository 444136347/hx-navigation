<div>
    <div class="alert alert-info alert-info-div" role="alert">
        <h5><span class="alert-info-span">当前导航标签：</span><span class="badge badge-primary">{{$tag->name}}</span></h5>
    </div>
</div>
            ...
            @foreach ($sites as $s)
                <div class="url-card col-12  col-sm-12 col-md-12 col-xl-12a col-xxl-12a">
                    <div class="url-body default">
                        <a onClick="jumpSite({{json_encode($s)}},1)" class="card no-c mb-4" data-toggle="tooltip"
                            ...
                            <div class="card-body">
                                ...
                                <div class="url-content d-flex align-items-center">
                                    <div class="url-img rounded-circle mr-2 d-flex align-items-center justify-content-center">
                                        <img class="lazy" src="{{getFullPath($s->cover_at)}}"
                                             data-src="{{getFullPath($s->cover_at)}}"
                                             onerror="javascript:this.src='{{getFullPath($s->cover_at)}}'"
                                             alt="{{$s->title}}">
                                    </div>
                        ...
                        <a onClick="jumpSite({{json_encode($s)}})" class="togo text-center text-muted is-views"
